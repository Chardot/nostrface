...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Future<void> _handlePrivateKeyInput() async {
    ...
    try {
      ...
      // Load the user's contact list from relays
      try {
        final publicKey = await keyService.getPublicKey();
        if (publicKey != null) {
          final profileService = ref.read(profileServiceProvider);
          await profileService.loadContactListFromRelays(publicKey);
          
          if (kDebugMode) {
            print('Contact list loaded for user: $publicKey');
          }
        }
      } catch (e) {
        if (kDebugMode) {
        ...
        // Continue with login even if contact list fails to load
      }
  ...
  Future<void> _handleGenerateKeys() async {
    ...
    try {
      ...
      // For a new key pair, there won't be any existing contact list
      // But we should still initialize the profile service
      final publicKey = await keyService.getPublicKey();
      if (publicKey != null) {
        final profileService = ref.read(profileServiceProvider);
        // This will find no contact list, but that's expected for new users
        await profileService.loadContactListFromRelays(publicKey);
      }
